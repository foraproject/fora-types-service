(function() {
    ...
    TypesService.prototype.buildTypeCache = function*(ctors, virtualTypeDefinitionsList) {
        var self = this;
        ...
        if (virtualTypeDefinitionsList) {
            for (var i = 0; i < virtualTypeDefinitionsList.length; i++) {
                ...
                for(var j = 0; j < vTypeDefs.typeDefinitions.length; j++) {
                    def = yield* self.completeVirtualTypeDefinition(vTypeDefs.typeDefinitions[j], vTypeDefs.ctor);
                    typeCache[def.name] = def;
                }
            }
        }
    ...
    TypesService.prototype.getTypeDefinitionFromCtor = function*(ctor) {
        if (ctor.typeDefinition) {
            if (!ctor.__typeDefinition) {
                ctor.__typeDefinition = yield* this.getTypeDefinition(ctor.typeDefinition.name);
                if (!ctor.__typeDefinition)
                    throw new Error("Unable to load type definition");
            }
            return ctor.__typeDefinition;
        } else {
            throw new Error("ctor does not have a valid type definition");
        }
    };



    TypesService.prototype.getTypeDefinitionFromObject = function*(obj, ctor) {
        if (obj && obj.getTypeDefinition)
            return yield* obj.getTypeDefinition();
        else
            return yield* this.getTypeDefinitionFromCtor(ctor);
    };
    ...
    TypesService.prototype.constructModelFromTypeDefinition = function*(obj, typeDefinition) {
        return yield* this.updateModel(null, obj, typeDefinition);
    };
            ...
            if (!this.modelServices.isModel || (this.modelServices.isModel && !this.modelServices.isModel(value))) {
                if (utils.isPrimitiveType(def.type)) {
                        ...
                        if (def.type === 'array') {
                            if (def.items.typeDefinition) {
                                ...
                                for (var _i = 0; _i < value.length; _i++) {
                                    var item = value[_i];
                                    arr.push(yield* this.constructModelFromTypeDefinition(item, def.items.typeDefinition));
                                }
                                value = arr;
                            }
                        }
                    ...
                    if (def.typeDefinition) {
                        if (value) {
                            value = yield* this.constructModelFromTypeDefinition(value, def.typeDefinition);
                        }
                    }
                }
            }
